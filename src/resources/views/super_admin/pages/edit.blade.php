...
@section('page_header')
    <div class="page__heading d-flex align-items-center">
        <div class="flex">
            ...
            <h1 class="m-0 h3">{{ __('messages.edit_page') }}</h1>
        </div>
        @if ($page->is_deletable)
            <a href="{{ route('super_admin.pages.delete', $page->id) }}" class="btn btn-danger ml-3 delete-confirm">
                <i class="material-icons">delete</i> 
                {{ __('messages.delete_page') }}
            </a>
        @endif
    </div>
@endsection
...
@section('content') 
    <form action="{{ route('super_admin.pages.update', $page->id) }}" method="POST" enctype="multipart/form-data">
