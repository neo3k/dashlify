@extends('layouts.app', ['page' => 'reports.product_sales'])

@section('title', __('messages.product_sales_report'))
        ...
        <div class="flex">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item" aria-current="page"><a href="{{ route('reports.product_sales', ['company_uid' => $currentCompany->uid]) }}">{{ __('messages.reports') }}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{ __('messages.product_sales_report') }}</li>
                </ol>
            </nav>
            <h1 class="m-0">{{ __('messages.product_sales_report') }}</h1>
        </div>
    </div>
@endsection 
...
@section('content') 
    <div class="row">
        ...
        <div class="col-12 col-md-4 text-right">
            <div class="btn-group mb-2">
                <a href="{{ route('reports.product_sales.pdf', [
    ...
    <div class="pdf-iframe">
        <iframe src="{{ route('reports.product_sales.pdf', [
            'company_uid' => $currentCompany->uid,
            'from' => isset(Request::get("filter")['from']) ? Request::get("filter")['from'] : \Carbon\Carbon::now()->format('Y-m-d'),
            'to' => isset(Request::get("filter")['to']) ? Request::get("filter")['to'] : \Carbon\Carbon::now()->addMonth()->format('Y-m-d')
